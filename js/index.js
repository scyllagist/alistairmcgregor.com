...
var margin = {
  ...
  height = y0 - margin.top - margin.bottom - 70;
  ...
  .map(function(y) {
    return d3.range(0, 40).map(function(x) {
      return {
        ...
        y0: y / 60,
        y:
          x < Math.random() * 4 - 2 + 13 || x > Math.random() * 4 - 2 + 27
            ? y / 60 + Math.random() / 7 / Math.abs(20 - x)
            : y / 60 + Math.sin(x / (Math.PI * 4)) * (Math.random() / 10)
      };
    });
  })
  .reverse();
...
function draw(data) {
  ...
  wave.append("path").attr("class", "line");
...
function move() {
  ...
  for (var y = 0; y < tmpData.length; y++) {
    for (var x = 0; x < tmpData[y].length; x++) {
      tmpData[y][x].y =
        origData[y][x].y0 +
        (origData[y][x].y - origData[y][x].y0) * y1 / height;
    }
  }
  draw(tmpData);
}
